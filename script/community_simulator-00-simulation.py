...
migration_function_list = migration_function_list.loc[migration_function_list.AlgorithmName.isin(['migrate_first_half', 'no_migration'])]
...
write_composition = True # Whether to write the species composition or not
...
for index_CommunityPhenotype in range(community_phenotype_list.shape[0]):         # Community phenotype
    for index_SelectionFunction in tqdm(range(selection_function_list.shape[0])): # Selection function
        for index_MigrationFunction in range(migration_function_list.shape[0]):   # Migration function 
            ...
            phenotype_algorithmID = community_phenotype_list.iloc[index_CommunityPhenotype]['AlgorithmID']
            
            # Pick selection algorithm
            selection_algorithm = selection_function_list.iloc[index_SelectionFunction]['AlgorithmName']
            selection_algorithmID = selection_function_list.iloc[index_SelectionFunction]['AlgorithmID']
            
            # Pick migration algrothms
            migration_algorithm = migration_function_list.iloc[index_MigrationFunction]['AlgorithmName']
            migration_algorithmID = migration_function_list.iloc[index_MigrationFunction]['AlgorithmID']
            
            # Run simulation
            print("\nphenotype algorithm: " + phenotype_algorithm + "\nselection algorithm: " + selection_algorithm + "\nmigration algorithm: " + migration_algorithm)
            try:
                # Set write_composition = True to record community compostion along transfer 
                function_time_df = simulate_community(assumptions, phenotype_algorithm, selection_algorithm, migration_algorithm, n = n_wells, n_propagation = n_propagation,  dilution=dilution, replicate_index = rr, write_composition=write_composition)
            except:
